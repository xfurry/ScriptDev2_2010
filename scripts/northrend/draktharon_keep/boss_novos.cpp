    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (Phase2 == true)
        {
            ...
            {     
                if (ArcaneCount == 2)
                {
                    DoCast(m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0),  m_bIsRegularMode ? SPELL_ARCANE_BLAST : H_SPELL_ARCANE_BLAST);
                    ArcaneCount = 0;
                }
...
void AddSC_boss_novos()
{
    ...
    pNewScript = new Script;
    pNewScript->Name = "crystal_handler";
    pNewScript->GetAI = &GetAI_crystal_handler;
    pNewScript->RegisterSelf();

    pNewScript = new Script;
    pNewScript->Name = "crystal_channel";
    pNewScript->GetAI = &GetAI_crystal_channel;
    pNewScript->RegisterSelf();

    pNewScript = new Script;
    pNewScript->Name = "mob_novos_minionAI";
    pNewScript->GetAI = &GetAI_mob_novos_minionAI;
    pNewScript->RegisterSelf();
}
