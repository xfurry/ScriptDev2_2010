...
struct MANGOS_DLL_DECL boss_grobbulusAI : public ScriptedAI
{
    ...
    {
        if ((spell->Id == SPELL_SLIME_SPRAY || spell->Id == SPELL_SLIME_SPRAY_H) && pTarget->GetTypeId() == TYPEID_PLAYER)
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        // Slime Stream Timer is used as count-douwn timer
        if (!m_uiSlimeStreamTimer)
        {
            if (!m_creature->IsWithinDistInMap(m_creature->getVictim(), ATTACK_DISTANCE))
                if (DoCastSpellIfCan(m_creature, SPELL_SLIME_STREAM) == CAST_OK)
                    ...
                    m_uiSlimeStreamTimer = 5*IN_MILLISECONDS;
        }
        else
        {
        ...
        if (m_uiInjectionTimer < uiDiff)
        {
            ...
            if (pTarget && !pTarget->HasAura(SPELL_MUTATING_INJECTION) && DoCastSpellIfCan(pTarget, SPELL_MUTATING_INJECTION) == CAST_OK)
            {
                // Timer dependend on time of encounter  - on enrage time between 5-8s, heroic 2-5s (TODO no reliable source for this)
                if (m_bIsRegularMode)
