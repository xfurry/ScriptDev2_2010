...
{ 
    ...
    {"We are ready to fight the next challenge!",   GOSSIP_ACTION_INFO_DEF+2,   false,  TYPE_ARGENT_CHAMPION},         //  summon argent challenge
...
struct MANGOS_DLL_DECL mob_toc_argent_trashAI: public ScriptedAI
{
	...
	void AttackStart(Unit* pWho)
    {
        if (m_creature->Attack(pWho, true)) 
        {
			...
			if(m_creature->GetEntry() == NPC_ARGENT_PRIESTESS)
