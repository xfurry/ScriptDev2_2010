...
struct MANGOS_DLL_DECL boss_eadricAI: public ScriptedAI
{
    ...
    {
        if (m_pInstance)
            m_pInstance->SetData(TYPE_ARGENT_CHAMPION, NOT_STARTED);
    }
    ...
    void AttackStart(Unit* pWho)
    {
        ...
        if (m_pInstance->GetData(TYPE_ARGENT_CHAMPION) != IN_PROGRESS) 
            return;
...
struct MANGOS_DLL_DECL mob_toc_memoryAI: public ScriptedAI
{
    ...
    {
        if (m_pInstance && m_pInstance->GetData(TYPE_ARGENT_CHAMPION) != IN_PROGRESS) 
